...
static naginata_kanamap ngdickana[] = {
    ...
    {.shift = NONE, .douji = B_M | B_E | B_P, .kana = {T, E, X, Y, O, NONE}, .func = nofunc},             // てゅ
    ...
    {.shift = NONE, .douji = B_V | B_SEMI | B_P, .kana = {F, Y, O, NONE, NONE, NONE}, .func = nofunc},    // ふゅ
    ...
    {.shift = NONE, .douji = B_F | B_L | B_P, .kana = {V, U, X, Y, O, NONE}, .func = nofunc},             // ゔゅ
...
bool naginata_press(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event)
{
    ...
    case SEMI:
        ...
        // 連続シフト
        static uint32_t rs[10][2] = {{D, F}, {C, V}, {J, K}, {M, COMMA}, {SPACE, 0}, {ENTER, 0}, {F, 0}, {V, 0}, {J, 0}, {M, 0}};
